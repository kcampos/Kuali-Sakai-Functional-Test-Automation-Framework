...
describe "Smoke Tests" do
  ...
  before :all do
    ...
    @user4 = @config.directory['person8']['id']
    @pass4 = @config.directory['person8']['password']
    @user4_name = "#{@config.directory['person8']['firstname']} #{@config.directory['person8']['lastname']}"
    @user5 = @config.directory['person9']['id']
    @pass5 = @config.directory['person9']['password']
    @user5_name = "#{@config.directory['person9']['firstname']} #{@config.directory['person9']['lastname']}"
    ...
    @document_name = "dUDYpV0QWY"#random_alphanums
    ...
    @existing_document = {:name=>@document_name, :title=>"Existing Doc", :visible =>"Participants only" }
  ...
  xit "User 1 invites contacts" do
  ...
  xit "Other users accept User 1's invite" do
  ...
  xit "User 2 sends User 1 a message" do
  ...
  xit "User 1 reads message inbox" do
    dash = @sakai.login(@user1, @pass1)
  ...
  xit "User 1 creates a private document" do
  ...
  it "Navigates through Messages, Library, Memberships, and Contacts" do
