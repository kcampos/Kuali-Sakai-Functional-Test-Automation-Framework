...
class SakaiOAE
  ...
  def sign_out
    ...
    sleep 0.2 #FIXME
    @browser.div(:text=>"Recent activity").wait_until_present
...
module PageObject
  module Accessors
    ...
    # link...
    def float_menu(name, menu_title, link_title, text, target_class)   
      define_method(name) {
    ...
    def navigating_button(name, id, class_name=nil)
      define_method(name) { 
          ...
          @browser.button(:id=>id).wait_while_present
          sleep 0.2 #FIXME
    ...
    def navigating_link(name, link_text, page_text=nil, class_name=nil)
      define_method(name) { 
          @browser.link(:text=>/#{Regexp.escape(link_text)}/).click
          unless page_text==nil
            @browser.wait_until { @browser.text.include? page_text }
          else
            sleep 1 # FIXME
          end
          unless class_name==nil
